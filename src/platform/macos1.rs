...
use io_kit_sys::{
    ...
    IORegistryEntryCreateCFProperty, IORegistryEntryCreateIterator, IORegistryEntryGetPath,
...
impl MacosMonitor {
    ...
    /// 执行单次查找
    unsafe fn find_modem_paths_once(
        device_service: io_service_t,
        ...
        while {
            ...
            if cu_path.is_none() {
                if let Some(path) =
                    unsafe { Self::get_ioreg_string(child_service, K_IO_CALLOUT_DEVICE) }
                {
                    cu_path = Some(path);
                }
            }
            if tty_path.is_none() {
                if let Some(path) =
                    unsafe { Self::get_ioreg_string(child_service, K_IO_DIALIN_DEVICE) }
                {
                    tty_path = Some(path);
                }
            }
    ...
    fn find_modem_paths_with_retry(
        device_service: io_service_t,
    ) -> (Option<String>, Option<String>) {
...
unsafe extern "C" fn device_callback(ref_con: *mut c_void, iterator: io_iterator_t) {
                ...
                for rule in &ctx.rules {
                    if let Some(method) = rule.matches(&dev) {
                        if !active_roles.contains_key(&rule.role) {
                            ...
                            ctx.tx
                                .send(DeviceEvent::Attached(ResolvedDevice {
...
impl DeviceMonitor for MacosMonitor {
    ...
    fn start(&self, rules: Vec<DeviceRule>, tx: Sender<DeviceEvent>) -> Result<()> {
        ...
        thread::spawn(move || unsafe {
            ...
            let matching_dict_add =
                IOServiceMatching(K_IOUSB_DEVICE_CLASS_NAME.as_ptr() as *const i8);
